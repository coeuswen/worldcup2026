...
<div class="section">
  ...
  <div class="analysis-text">
    ...
    本报告首次将赔率三项联动分析(胜平负组合模式)纳入融合框架: 每场比赛的赔率组合被归入六种模式(低胜高平/中庸/低负高平等), 平赔"诚实度"与ELO独立概率交叉验证, 联赛特性(世界杯中立场地)校准。融合权重动态分配: ELO权重~60-70%, 战术推演~15-25%, 市场赔率~10-20%。<strong>此外, 平局基线修正项(默认28%, 大ELO差距场次下调至20-24%)作为正则化因子防止过度自信, 这可能导致融合胜率略低于纯ELO值(如荷兰52%&lt;ELO56%)——此为设计特性而非数学矛盾。</strong>当三项联动信号与ELO一致时(如德国高平排除平局), 融合信度提升。
  </div>
</div>
